...
treemut_path=opt$treemut_path

#----------------------------------
# Load packages (install if they are not installed yet)
#----------------------------------
options(stringsAsFactors = F)
cran_packages=c("ggplot2","ape","seqinr","stringr","data.table","tidyr","dplyr","VGAM","MASS")
bioconductor_packages=c("ggtree","Rsamtools","GenomicRanges")
...
if(split_trees&mut_id=="both"){
  ...
  edge_length[names(edge_length_nonzero)]=edge_length_nonzero
  tree$edge.length=as.numeric(edge_length)
  p1=  ggtree(tree) + geom_tiplab(aes(x=branch),vjust=-0.3)+theme_tree2()+xlim(0,max(fortify(tree)$x)*1.3)
  ...
  edge_length[names(edge_length_nonzero)]=edge_length_nonzero
  tree$edge.length=as.numeric(edge_length)
  p2=  ggtree(tree) + geom_tiplab(aes(x=branch),vjust=-0.3)+theme_tree2()+xlim(0,max(fortify(tree)$x)*1.3)
  ...
  p2=ggtree(tree_collapsed) + geom_tiplab(aes(x=branch),vjust=-0.3)+xlim(0,max(fortify(tree_collapsed)$x)*1.3)
